...
/*
 * A is driving his favorite vehicle on the 101 freeway. He notices that the check engine light of his vehicle is on, and he wants to service it immediately to avoid any risks. Luckily, a service lane runs parallel to the highway. The length of the service lane is NN units. The service lane consists of NN segments of equal length and different width.

A can enter to and exit from any segment. Let's call the entry segment as index ii and the exit segment as index jj. Assume that the exit segment lies after the entry segment (i≤ji≤j) and 0≤i0≤i. A has to pass through all segments from index ii to index jj (both inclusive).

Paradise Highway

A has three types of vehicles - bike, car, and truck - represented by 11, 22 and 33, respectively. These numbers also denote the width of the vehicle.
...
Given the entry and exit point of A's vehicle in the service lane, output the type of the largest vehicle which can pass through the service lane (including the entry and exit segments).
...
TT test cases follow. Each test case contains two integers, ii and jj, where ii is the index of the segment through which A enters the service lane and jj is the index of the lane segment through which he exits.
...
Note: A has to pass through all segments from index ii to index jj (both inclusive).
